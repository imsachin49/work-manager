...
const Tasks = ({task}) => {
  ...
  const handleDelete = async () => {
    try {
      const res = await axios.delete(`https://work-manage-49.vercel.app/api/tasks/${task._id}`, {
  ...
  const handleSave = async (e) => {
    ...
    if(editTitle===''){
      ...
      try {
        const res = await axios.put(`https://work-manage-49.vercel.app/api/tasks/${task._id}`,{ title: editTitle },
          {headers: { Authorization: `Bearer ${token}` },}
        );
